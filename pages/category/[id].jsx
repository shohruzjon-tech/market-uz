...
const ProductSearchResult = () => {
  ...
  useEffect(()=>{
    ...
    dispatch(getCategoryListStart());
        getProductsWithCategoryId(parseInt(router?.query?.id)).then((res)=>{
            setProducts(res);
            setLoading(false)
          })
  }, [router?.query?.id]);
  ...
  return <NavbarLayout>
      <Box pt={2.5}>
        ...
        p: {
          ...
          <div>
            <H5>Barcha mahsulotlar</H5>
            <Paragraph color="grey.600">Ushbu kategoriyada <strong>{filteredProducts?.length}</strong> ta mahsulot bor</Paragraph>
          </div>
